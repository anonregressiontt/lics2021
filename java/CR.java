...
/**
 * Constructive real numbers, also known as recursive, or computable reals. Each
 * recursive real number is represented as an object that provides an
 * approximation function for the real number. The approximation function
 * guarantees that the generated approximation is accurate to the specified
 * precision. Arithmetic operations on constructive reals produce new such
 * objects; they typically do not perform any real computation. In this sense,
 * arithmetic computations are exact: They produce a description which describes
 * the exact answer, and can be used to later approximate it to arbitrary
 * precision.
 * <P>
 * When approximations are generated, <I>e.g.</i> for output, they are accurate
 * to the requested precision; no cumulative rounding errors are visible. In
 * order to achieve this precision, the approximation function will often need
 * to approximate subexpressions to greater precision than was originally
 * demanded. Thus the approximation of a constructive real number generated
 * through a complex sequence of operations may eventually require evaluation to
 * very high precision. This usually makes such computations prohibitively
 * expensive for large numerical problems. But it is perfectly appropriate for
 * use in a desk calculator, for small numerical problems, for the evaluation of
 * expressions computated by a symbolic algebra system, for testing of accuracy
 * claims for floating point code on small inputs, or the like.
 * <P>
 * We expect that the vast majority of uses will ignore the particular
 * implementation, and the member functons <TT>approximate</tt> and
 * <TT>get_appr</tt>. Such applications will treat <TT>CR</tt> as a conventional
 * numerical type, with an interface modelled on <TT>java.math.BigInteger</tt>.
 * No subclasses of <TT>CR</tt> will be explicitly mentioned by such a program.
 * <P>
 * All standard arithmetic operations, as well as a few algebraic and
 * transcendal functions are provided. Constructive reals are immutable; thus
 * all of these operations return a new constructive real.
 * <P>
 * A few uses will require explicit construction of approximation functions. The
 * requires the construction of a subclass of <TT>CR</tt> with an overridden
 * <TT>approximate</tt> function. Note that <TT>approximate</tt> should only be
 * defined, but never called. <TT>get_appr</tt> provides the same functionality,
 * but adds the caching necessary to obtain reasonable performance.
 * <P>
 ...
 * <P>
 * Any operation may also throw <TT>com.sgi.math.PrecisionOverflowError</tt> If
 * the precision request generated during any subcalculation overflows a 28-bit
 * integer. (This should be extremely unlikely, except as an outcome of a
 * division by zero, or other erroneous computation.)
 * 
 */
public abstract class CR extends Number {
	...
	/**
	 * Setting this to true requests that all computations be aborted by throwing
	 * AbortedError. Must be rest to false before any further computation. Ideally
	 * Thread.interrupt() should be used instead, but that doesn't appear to be
	 * consistently supported by browser VMs.
	 */
	public volatile static boolean please_stop = false;

	/**
	 * Must be defined in subclasses of <TT>CR</tt>. Most users can ignore the
	 * existence of this method, and will not ever need to define a <TT>CR</tt>
	 * subclass. Returns value / base ** prec rounded to an integer. The error in
	 * the result is strictly < 1. Informally, approximate(n) gives a scaled
	 * approximation accurate to 2**n. Implementations may safely assume that
	 * precision is at least a factor of 8 away from overflow.
	 */
	...
	/**
	 * The constructive real number corresponding to a <TT>BigInteger</tt>.
	 */
	...
	/**
	 * The constructive real number corresponding to a Java <TT>int</tt>.
	 */
	...
	/**
	 * The constructive real number corresponding to a Java <TT>long</tt>.
	 */
	...
	/**
	 * The constructive real number corresponding to a Java <TT>double</tt>. The
	 * result is undefined if argument is infinite or NaN.
	 */
	...
	/**
	 * The constructive real number corresponding to a Java <TT>float</tt>. The
	 * result is undefined if argument is infinite or NaN.
	 */
	...
	/**
	 * Returns value / 2 ** prec rounded to an integer. The error in the result is
	 * strictly < 1. Produces the same answer as <TT>approximate</tt>, but uses and
	 * maintains a cached approximation. Normally not overridden, and called only
	 * from <TT>approximate</tt> methods in subclasses. Not needed if the provided
	 * operations on constructive reals suffice.
	 */
	...
	// answer is < n.
	int msd(int n) {
		if (!appr_valid || max_appr.compareTo(big1) <= 0 && max_appr.compareTo(bigm1) >= 0) {
			get_appr(n - 1);
	...
	// Public operations.
	/**
	 * Return 0 if x = y to within the indicated tolerance, -1 if x < y, and +1 if x
	 * > y. If x and y are indeed equal, it is guaranteed that 0 will be returned.
	 * If they differ by less than the tolerance, anything may happen. The tolerance
	 * allowed is the maximum of (abs(this)+abs(x))*(2**r) and 2**a
	 * 
	...
	/**
	 * Approximate comparison with only an absolute tolerance. Identical to the
	 * three argument version, but without a relative tolerance. Result is 0 if both
	 * constructive reals are equal, indeterminate if they differ by less than 2**a.
	 *
	 * @param x The other constructive real
	 * @param a Absolute tolerance in bits
	 */
	...
	/**
	 * Should be called only if <TT>x != y</tt>. Return -1 if <TT>this < x</tt>, or
	 * +1 if <TT>this > x</tt>. If <TT>this == x</tt>, this will not terminate
	 * correctly; typically it will run until it exhausts memory. If the two
	 * constructive reals may be equal, the two or 3 argument version of compareTo
	 * should be used.
	 */
	...
	/**
	 * Should be called only if <TT>x != 0</tt>. Return -1 if negative, +1 if
	 * positive. In the 0 case, this will not terminate correctly; typically it will
	 * run until it exhausts memory. If the two constructive reals may be equal, the
	 * one or two argument version of signum should be used.
	 */
	public int signum() {
		for (int a = -20;; a *= 2) {
			check_prec(a);
	...
	/**
	 * Return the constructive real number corresponding to the given textual
	 * representation and radix.
	 *
	 * @param s     [-] digit* [. digit*]
	 * @param radix
	 */
	public static CR valueOf(String s, int radix) throws NumberFormatException {
		int len = s.length();
	 ...
	 * @param s [-] digit* [. digit*]
	 */
	public static CR valueOf(String s) throws NumberFormatException {
		return valueOf(s, 10);
	}

	/**
	 * Return a textual representation accurate to <TT>n</tt> places to the right of
	 * the decimal point. <TT>n</tt> must be nonnegative.
	 *
	...
	/**
	 * Return a double which differs by less than one in the least represented bit
	 * from the constructive real.
	 */
	public double doubleValue() {
		int my_msd = iter_msd(-1080 /* slightly > exp. range */);
		if (Integer.MIN_VALUE == my_msd)
			return 0.0;
	...
	/**
	 * Return a float which differs by less than one in the least represented bit
	 * from the constructive real.
	 */
	...
	/**
	 * The multiplicative inverse of a constructive real. <TT>x.inverse()</tt> is
	 * equivalent to <TT>CR.valueOf(1).divide(x)</tt>.
	 */
	public CR inverse() {
	 ...
	 * Requires <TT>x</tt> = <TT>y</tt> if <TT>this</tt> = 0. Since comparisons may
	 * diverge, this is often a useful alternative to conditionals.
	 */
	...
	/**
	 * The absolute value of a constructive reals. Note that this cannot be written
	 * as a conditional.
	 */
	public CR abs() {
	...
	public static CR two = valueOf(2);

	/**
	 * The ratio of a circle's circumference to its diameter.
	 */
	public static CR PI = four.multiply(four.multiply(atan_reciprocal(5)).subtract(atan_reciprocal(239)));
...
//This assumes that the order of magnitude of the number is rougxhly one.
//
abstract class slow_CR extends CR {
	...
	public BigInteger get_appr(int precision) {
		...
		if (appr_valid && precision >= min_prec) {
			...
			int eval_prec = (precision >= max_prec ? max_prec : (precision - prec_incr + 1) & ~(prec_incr - 1));
...
class select_CR extends CR {
	...
	protected BigInteger approximate(int p) {
		if (selector_sign < 0)
			return op1.get_appr(p);
		if (selector_sign > 0)
			return op2.get_appr(p);
...
class inv_CR extends CR {
	...
	protected BigInteger approximate(int p) {
		...
		BigInteger adj_dividend = dividend.add(abs_scaled_divisor.shiftRight(1));
...
class prescaled_exp_CR extends CR {
	...
	protected BigInteger approximate(int p) {
		...
		int calc_precision = p - bound_log2(2 * iterations_needed) - 4; // for error in op, truncation.
		int op_prec = p - 3;
		...
		int n = 0;
		BigInteger max_trunc_error = big1.shiftLeft(p - 4 - calc_precision);
...
class integral_atan_CR extends slow_CR {
	...
	protected BigInteger approximate(int p) {
		...
		// exclusive of error in op.
		int calc_precision = p - bound_log2(2 * iterations_needed) - 2; // for error in op, truncation.
		// Error in argument results in error of < 3/8 ulp.
		...
		while (current_term.abs().compareTo(max_trunc_error) >= 0) {
			...
			current_term = current_power.divide(BigInteger.valueOf(current_sign * n));
			current_sum = current_sum.add(current_term);
		}
		return scale(current_sum, calc_precision - p);
	}
...
class sqrt_CR extends CR {
	...
	protected BigInteger approximate(int p) {
		...
		if (result_digits > fp_prec) {
			...
			BigInteger prod_prec_scaled_numerator = last_appr.multiply(last_appr).add(op_appr);
			BigInteger scaled_numerator = scale(prod_prec_scaled_numerator, appr_prec - p);
			...
			BigInteger scaled_bi_appr = op.get_appr(op_prec).shiftLeft(fp_op_prec);
